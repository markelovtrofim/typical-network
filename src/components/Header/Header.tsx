    ...
    <header className={styles.general}>
      <div className="container">
        <div className={styles.inner}>
          <NavLink className={styles.logo_link} to="/" title="Главная страница"><h1
            className={styles.logo_left}>TYPICAL</h1><h1 className={styles.logo_right}>NETWORK</h1></NavLink>
          <nav>
            {state.navData.map((d: any) => <NavLink className={styles.link} activeClassName={styles.active} key={d.id}
                                                    to={d.link}><span>{d.text}</span></NavLink>)}
          </nav>
